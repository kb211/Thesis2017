...
def mle(features, targets, alpha=0.0001):
    ...
    pos_len = pos.shape[0] + alpha
    neg_len = features.shape[0] - pos_len + alpha
    pos = sum(pos) + alpha
    neg = sum(features[targets[:, 0] == 1, :]) + alpha

    return np.asmatrix([(neg/ neg_len), (pos / pos_len)]).transpose(), [float(neg_len) / (pos_len + neg_len), float(pos_len) / (pos_len + neg_len)]

def mle2(features):
    return np.sum(features, axis=0)/ features.shape[0]

def mle3(features, targets, alpha=0.0001):
...
if __name__ == '__main__':
    ...
    evaluationMLE = evaluation()
    evaluationEM = evaluation()
    ...
    print x_train_nonfraud.shape, x_train_fraud.shape
    ...
    for n_samples in [1051, 1051 * 2, 1051 * 4, 1051 * 8, 1051 * 16, 1051 * 32]:
        p_c = np.load('pickles/p_c(nonfraud)' + str(FLAGS.n_clusters) + "_" + str(n_samples) + '.npy')
        x_given_c = np.load('pickles/x_given_c(nonfraud)' + str(FLAGS.n_clusters)+ "_" + str(n_samples) + '.npy')
        ...
        #np.save('pickles/p_c(nonfraud)' + str(FLAGS.n_clusters) + "_" + str(n_samples), p_c)
        #np.save('pickles/x_given_c(nonfraud)'+ str(FLAGS.n_clusters) + "_" + str(n_samples), x_given_c)

        print "alpha: ", 0, "n_samples: ", n_samples
        x_given_f, p_f = mle3(x_train_undersample, y_train_undersample, 0)

        print "MLE: "
        predict(x_given_f, p_f, x_test, y_test, n_samples)

        print "MLE + EM: "
        params = [p_c, x_given_c, x_given_f, p_f]
